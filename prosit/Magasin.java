...
public void ajouterProduit (Produit p) throws MgPleinException, PrixNegatifException {

        if (p.getPrix() < 0) {
            throw new PrixNegatifException("Prix de " + p.getLibelle() + " est négatif !!");
        } else {
            if (this.exist(p) == false) {
                if (this.getNbrProduit() == 2) {
                    try {
                        produits[this.getNbrProduit()] = p;
                    } 
                    catch (ArrayIndexOutOfBoundsException ex) {
                        throw new MgPleinException("mg plein");
                    }
                }
            } else {
                System.out.println("produit déjà existe");
            }
        }
